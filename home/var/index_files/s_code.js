...
/************************ ADDITIONAL FEATURES ************************
     Plugins
*/
var sc_code_ver="H.24.2_230112"
var s_account="rcsglobal"
...
s.linkInternalFilters="javascript:,corriere.it,iodonna.it"
...
s._channelDomain="Social Media|facebook.com,myspace.com,twitter.com,linkedin.com,wikio,netvibes.com,youtube.com>Network RCS|corriere.it,gazzetta.it";
...
function s_doPlugins(s) {
  ...
  if(!s.eVar22) s.eVar22=s.getQueryParam('intcmp');
	...
	s.channelManager('cmpid',':','s_cm_COR');
  var tmpKey=s._keywords?decodeURI(s._keywords).toLowerCase():'';
  if(s._referringDomain) s.eVar50=s._referringDomain;
  if(s._campaignID){
    var tmpCmp=s.getQueryParam('s_kw');
    s.campaign=s._campaignID+(tmpCmp?'|':'')+tmpCmp;
    if(tmpKey) s.eVar47=tmpKey; //paid kwd
  } else if(tmpKey) s.eVar48=tmpKey; //natural kwd
  if(s.eVar22&&s.eVar22.toLowerCase().indexOf('em')>-1) s._channel="Newsletter RCS";
  if(s._channel) s.eVar49=s.eVar45=s._channel;
  //Search Engine by Channel - non-deduped
  s.channelManager('cmpid',':','0');
  if(s._channel=='Paid Search') s.prop46=s._referringDomain;
  else if(s._channel=='Natural Search') s.prop47=s._referringDomain;
...
/*
 * channelManager v2.45 - Tracking External Traffic - Includes encrypted search update
 */
